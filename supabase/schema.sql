...
drop policy if exists "Authenticated users create purchased reviews" on public.reviews;
...
create policy "Authenticated users create purchased reviews"
on public.reviews
for insert
to authenticated
with check (
  ...
  and exists (
    ...
    where public.orders.user_id = auth.uid()
      and public.order_items.product_id = product_id
  )
);
